...
struct WandData {
    explicit WandData(CLI::App* app)
    {
        ...
        app->add_flag("--compressed-wand", m_wand_compressed, "Compressed WAND data file")->needs(wand);
    ...
    void apply_shard(pisa::Shard_Id shard)
    {
        if (m_wand_data_path) {
...
struct Query {
    explicit Query(CLI::App* app)
    {
        ...
        app->add_option("--stopwords", m_stop_words, "List of blacklisted stop words to filter out")
...
CLI::Option* add_scorer_options(CLI::App* app, T& args, ScorerMode scorer_mode)
{
    ...
    if (scorer_mode == ScorerMode::Required) {
        scorer = app->add_option("-s,--scorer", args.m_params.name, "Scorer function")->required();
    } else {
...
struct Invert {
    ...
    [[nodiscard]] auto term_count() const -> std::optional<std::uint32_t> { return m_term_count; }
...
struct CreateWandData {
    ...
    [[nodiscard]] auto output() const -> std::string { return m_output; }
    [[nodiscard]] auto scorer_params() const { return m_params; }
    [[nodiscard]] auto block_size() const -> pisa::BlockSize
    {
        if (m_lambda) {
            spdlog::info("Lambda {}", *m_lambda);
            return pisa::VariableBlock(*m_lambda);
        }
        spdlog::info("Fixed block size: {}", *m_fixed_block_size);
        return pisa::FixedBlock(*m_fixed_block_size);
    }
...
struct ReorderDocuments {
    ...
    [[nodiscard]] auto document_lexicon() const -> std::optional<std::string> { return m_doclex; }
    ...
    [[nodiscard]] auto input_collection() const -> pisa::binary_freq_collection
    {
        return pisa::binary_freq_collection(input_basename().c_str());
    }
    ...
    [[nodiscard]] auto node_config() const -> std::optional<std::string> { return m_node_config; }
...
using CompressArgs = Args<arg::Compress, arg::Encoding, arg::Quantize<arg::ScorerMode::Optional>>;
using CreateWandDataArgs = Args<arg::CreateWandData>;

struct TailyStatsArgs: Args<arg::WandData<arg::WandMode::Required>, arg::Scorer> {
    explicit TailyStatsArgs(CLI::App* app)
        : Args<arg::WandData<arg::WandMode::Required>, arg::Scorer>(app)
    {
...
struct TailyRankArgs: Args<arg::Query<arg::QueryMode::Ranked>> {
    explicit TailyRankArgs(CLI::App* app) : Args<arg::Query<arg::QueryMode::Ranked>>(app)
    {
...
struct TailyThresholds: Args<arg::Query<arg::QueryMode::Ranked>> {
    explicit TailyThresholds(CLI::App* app) : Args<arg::Query<arg::QueryMode::Ranked>>(app)
    {
    ...
    /// Transform paths for `shard`.
    void apply_shard(pisa::Shard_Id shard) { m_stats = expand_shard(m_stats, shard); }
