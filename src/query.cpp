...
//                       uint64_t k, std::string const &documents_filename,
//                       ScorerParams const &scorer_params,
//                       std::string const &run_id, std::string const
//                       &iteration) {
...
//       ranked_and_q(make_scored_cursors(index, *scorer, query),
//                    index.num_docs());
//       topk.finalize();
//       return topk.topk();
//     };
...
//       ranked_or_q(make_scored_cursors(index, *scorer, query),
//       index.num_docs()); topk.finalize(); return topk.topk();
//     };
...
//     query_fun = [&, accumulator = Simple_Accumulator(index.num_docs())](
//                     Query query) mutable {
//       topk_queue topk(k);
//       ranked_or_taat_query ranked_or_taat_q(topk);
//       ranked_or_taat_q(make_scored_cursors(index, *scorer, query),
//                        index.num_docs(), accumulator);
//       topk.finalize();
//       return topk.topk();
//     };
...
//   auto source =
//   std::make_shared<mio::mmap_source>(documents_filename.c_str()); auto docmap
//   = Payload_Vector<>::from(*source);

//   std::vector<std::vector<std::pair<float, uint64_t>>> raw_results(
//       queries.size());
//   auto start_batch = std::chrono::steady_clock::now();
//   tbb::parallel_for(size_t(0), queries.size(),
//                     [&, query_fun](size_t query_idx) {
...
//       std::cout << fmt::format(
//           "{}\t{}\t{}\t{}\t{}\t{}\n",
//           qid.value_or(std::to_string(query_idx)), iteration,
//           docmap[result.second], rank, result.first, run_id);
//     }
//   }
//   auto end_print = std::chrono::steady_clock::now();
//   double batch_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
//                         end_batch - start_batch)
//                         .count();
//   double batch_with_print_ms =
//       std::chrono::duration_cast<std::chrono::milliseconds>(end_print -
//                                                             start_batch)
//           .count();
//   spdlog::info("Time taken to process queries: {}ms", batch_ms);
//   spdlog::info("Time taken to process queries with printing: {}ms",
//                batch_with_print_ms);
// }
