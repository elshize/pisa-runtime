...
using ProcFunc = std::function<std::vector<std::pair<float, uint64_t>>(pisa::Query)>;

class QueryProcessor {
  public:
    virtual auto process_query(pisa::Query) -> std::vector<std::pair<float, uint64_t>> = 0;
    ...
    -> ProcFunc
{
    ...
    std::function<std::vector<std::pair<float, uint64_t>>(pisa::Query)> query_fun;
